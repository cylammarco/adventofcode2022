# Needed help from https://galaxyinferno.com/how-to-solve-advent-of-code-2022-day-7-with-python/
        ...
        if self.is_dir:
            for child in self.children:
                if child.is_dir and child.get_size() <= 100000:
                    dir_sizes += (
                        child.get_size() + child.find_subdirectories_part1()
                    )
                else:
        ...
        if self.is_dir:
            for child in self.children:
                if child.is_dir and child.get_size() >= min_size:
                    dir_sizes += [
                        child.get_size()
                    ] + child.find_subdirectories_part2(min_size)
                else:
